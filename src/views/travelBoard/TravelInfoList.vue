<template>
  <div class="board-container">
    ...
    <div class="board-selector">
      <button @click="fetchtravelList('info', 1)" :class="{'active': currentBoardType === 'info'}">{{ countryName }}여행 정보</button>
...
export default {
  ...
  methods: {
    async fetchtravelList(boardType, page = 1) {
      ...
      try {
        const countryCode = this.$route.params.countryCode; // URL에서 countryCode 가져오기
        const response = await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/travel/travelBoard/list`, {
          params: {
            countryCode,   // 쿼리 파라미터로 전달
            boardType,     // 쿼리 파라미터로 전달
