<template>
  <div class="container">
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="(row, id) in humorList" :key="id">
          <td>{{ row.id }}</td>
...
export default {
  ...
  methods: {
    ...
    onPostClicked(row) {
      this.$router.push({ path: '../humorBoard/humorDetail', query: { id: row.id } });
      this.increaseViewCount(row.id);
    },
    increaseViewCount(id) {
      this.$axios.put(`${process.env.VUE_APP_SERVER_URL}/api/humor/increaseViewCount/${id}`).then(() => {
        console.log('조회수 증가');
      }).catch(err => {
