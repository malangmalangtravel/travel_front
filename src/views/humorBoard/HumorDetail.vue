<template>
  <div class="board-detail-container">
    ...
    <!-- HumorList 컴포넌트에 현재 게시글 ID를 전달 -->
    <HumorList :currentBoardId="boardDetail.id" :initialBoardCode="'humor'" />
    ...
    <div class="pagination-buttons-fixed">
      ...
      <router-link :to="nextBoardId ? `/humorBoard/humorDetail/${nextBoardId}?boardCode=${boardCode}` : ''">
        <button
      ...
      <router-link :to="previousBoardId ? `/humorBoard/humorDetail/${previousBoardId}?boardCode=${boardCode}` : ''">
        <button
...
import HumorList from './HumorList.vue';
...
export default {
  ...
  methods: {
    async fetchBoardDetail(id) {
      ...
      try {
        const response = await axios.get(`${process.env.VUE_APP_SERVER_URL}/api/humor/${boardCode}Board/detail/${id}`);
    ...
    async incrementViewCount(id) {
      try {
        await axios.post(`${process.env.VUE_APP_SERVER_URL}/api/humor/incrementViewCount/${id}`);
